...
def test_clean_program_passes():
    r = assess(hypothetical_pnl=[100, 10, 0, 0], hedged_item_pnl=[-100, -10, 0, 0])
    assert r.effective
...
def test_credit_dominance_rejected():
    ...
    r = assess(
        hypothetical_pnl=[100, 10, 0, 0],
        hedged_item_pnl=[-100, -10, 0, 0],
        market_pnl=[10, 5, 0, 0],
        credit_pnl=[100, 10, 0, 0],
    )  # credit variance >> market
...
def test_band_is_not_the_operative_test():
    ...
    inside = assess(
        hypothetical_pnl=[100, -10, 10, -10], hedged_item_pnl=[-50, -50, 0, 0]
    )
    ...
    outside = assess(
        hypothetical_pnl=[130, 13, 0, 0], hedged_item_pnl=[-100, -10, 0, 0]
    )
