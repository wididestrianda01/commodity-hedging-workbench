...
from hedging_workbench.stress import run_scenario, stressed_path, stress_report

def _base(last=300.0):
    return pd.Series([295.0, 297.0, last],
...
def test_collar_caps_funding_on_downside():
    ...
    contracts = 4.0
    base = _base(300.0)
    collar = zero_cost_collar(300.0, put_strike=280.0, t=0.25,
                              sigma=0.385, r=0.0366)
    r = run_scenario(contracts, base, -1.5, "backwardation_widening",
                     collar=collar)
...
def test_report_scenario_set_and_buffer():
    ...
    assert names == ["down_1sigma", "down_2sigma", "up_2sigma",
...
def test_real_frozen_inputs_smoke():
    ...
    out = stress_report(2.6667, kc, fit.garch_last / 100.0,
                        collateral_usd=37.9e6)
    assert len(out) == 4
