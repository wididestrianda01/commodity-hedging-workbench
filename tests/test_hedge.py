...
from hedging_workbench.conventions import CONTRACT_LB, DOLLARS_PER_CENT
from hedging_workbench.hedge import (
    build_program,
    exposure_schedule,
    roll_schedule,
    select_contracts,
...
def test_selection_rule_earliest_expiry_in_or_after_month():
    exp = select_contracts(exposure_schedule(37_500, "2026-10-01", 4), _synth_curve())
...
def test_selection_gap_flagged_past_last_contract():
    exp = select_contracts(exposure_schedule(37_500, "2027-04-01", 2), _synth_curve())
    assert exp["gap"].all()
    assert (exp["symbol"] == "KCH27.NYB").all()  # last contract fallback


def test_roll_schedule_transitions_and_dates():
    exp = select_contracts(exposure_schedule(37_500, "2026-10-01", 6), _synth_curve())
...
def test_variation_margin_hand_calculated():
    path = pd.Series([100.0, 102.0, 99.0], index=pd.date_range("2026-10-05", periods=3))
    vm = variation_margin(contracts=2.0, price_path=path, initial_margin=8_000.0)
    ...
    assert vm["balance_usd"].tolist() == pytest.approx([16_000.0, 17_500.0, 15_250.0])
