...
MAX_NAN_FRAC = 0.05
MAX_PRICE = {
    "coffee": 2000.0,  # cents/lb — coffee sanity ceiling
    "gold": 20000.0,
}  # $/oz — gold sanity ceiling
...
def main() -> None:
    from hedging_workbench.data.frozen import load, verify
    ...
    for symbol, stats in report.series.items():
        print(
            f"  {symbol:<12} rows={stats['rows']:<6} last={stats['last_close']:>9.2f}  {stats['last_date']}"
        )
    if not report.ok:
        raise SystemExit(1)
