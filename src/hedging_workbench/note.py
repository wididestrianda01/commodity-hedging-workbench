...
class ParticipationNote:
    ...
    def __post_init__(self):
        object.__setattr__(self, "strike",
                           self.f0 if self.strike is None else self.strike)
    ...
    def closed_form(self, sigma: float, r: float) -> dict:
        ...
        return {"bond": bond, "call": call / self.f0,
                "participation_value": upside, "price": bond + upside}
    ...
    def greeks(self, sigma: float, r: float, h: float = 1e-4) -> dict:
        ...
        d1 = (math.log(f0 / k) + 0.5 * sigma * sigma * t) / sq
        df = math.exp(-r * t)
        p = self.participation
        delta = -(p * df * norm.cdf(d1) / f0
                  - p * black76("call", f0, k, t, sigma, r) / f0 ** 2)
        vega = -p * df * norm.pdf(d1) * math.sqrt(t)
        return {"delta": delta, "vega": vega}

    def mc_price(self, sigma: float, r: float, n_paths: int = 100_000,
                 seed: int = 42) -> dict:
        ...
        """
        if n_paths < 2 or n_paths % 2:
            raise ValueError("n_paths must be even and >= 2")
        rng = np.random.default_rng(seed)
        half = n_paths // 2
        z = rng.standard_normal(half)
        drift = -0.5 * sigma * sigma * self.tenor
        sq = sigma * math.sqrt(self.tenor)
        ft = np.concatenate([
            self.f0 * np.exp(drift + sq * z),
            self.f0 * np.exp(drift - sq * z),
        ])
        ...
        return {"price": bond + est, "upside": est, "se": se,
                "ci95": (bond + est - 1.96 * se, bond + est + 1.96 * se)}

    def mc_greeks(self, sigma: float, r: float, n_paths: int = 100_000,
                  seed: int = 42, rel_bump: float = 0.01) -> dict:
        """Bump-and-reprice MC greeks, ISSUER side — validates `greeks`."""
        def price(f0: float, s: float) -> float:
            n = ParticipationNote(self.notional, f0, self.tenor,
                                  self.participation, self.strike)
        ...
        return {"delta": -(d0 - d1_) / (2 * f0 * rel_bump),
                "vega": -(u0 - u1_) / 0.02}


def render_governance(note: ParticipationNote, sigma: float, r: float,
                      underlying: str = "ICE Coffee C futures (KC=F)") -> str:
