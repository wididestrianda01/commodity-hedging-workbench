...
out["book"] = {"contracts": contracts, "epe": epe(prof), "pfe_1y": float(prof["pfe"].iloc[-1])}
ee = prof["ee"].to_numpy()
tn = np.linspace(1 / 252, 1.0, len(ee))
out["cva"] = {"ours": cva_unilateral(ee, tn, 0.03, 0.6, r)}

kc = load(["KC=F"])["KC=F"].dropna()
pnl = kc.diff().dropna() * DOLLARS_PER_CENT * contracts
ve = var_es(pnl)
out["var_es"] = {str(i): {"var": float(row["var"]), "es": float(row["es"])} for i, row in ve.iterrows()}

tbl = decision_from_frozen(
    volume_lb=1_200_000, contracts=contracts, put_strike=280.0
)
