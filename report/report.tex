...
VaR coverage (95\%) & 38 breaches vs 25.2 expected over 4 windows (ratio 1.51, band 0.5--1.5) & \textcolor{red}{FLAG} \\
...
\paragraph{The VaR coverage FLAG.} The historical 95\% VaR under-covers by
roughly 50\% on this book: 38 breaches against 25.2 expected over four
126-day windows. This is the classic unconditional-historical-VaR failure
mode on fat-tailed, volatility-clustered commodity returns --- the
trailing window's empirical quantile understates the tail of the next
window precisely when volatility has risen. SR 26-2's outcomes-analysis
pillar exists to surface exactly this; the honest response is recorded
here and the remediation path (volatility-scaled or filtered-historical
VaR, reusing the Phase 2 GARCH engine) is named in the limitations rather
than silently widening the acceptance band.
...
coffee options.
\item \textbf{VaR under-coverage is open.} The 95\% coverage FLAG
(Section~\ref{sec:validation}) stands; volatility-scaled VaR is the
pre-declared remediation, not yet implemented.
